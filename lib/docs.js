...
export const USAGE_DOCS = {
  quickStart: [
    ...
    '解析完成后依次查看第 2–5 步，最后导出 Markdown/PDF 报告。',
    '改完一轮优化后再采一次，把结果导入第 6 步：页面会把两次采集配对，在第 3、4 步左右并排对比，并逐条判定第 5 步的建议是否达成。',
  ],
  compareAdvice: [
    '前后对比要求两次采集**测的是同一件事**：同一命令、同一并发与输入长度、同一阶段口径。步数差异 >15%、Prefill/Decode 构成变化 >10pt、任一侧被采样或产物文件不同，都会在对比面板里明确告警。',
    '占比类指标的分母是墙钟：优化缩短墙钟后，"通信未掩盖占比"这类比值可能反而变大，判断好坏请以对应的 µs 绝对值为准（页面会同时给出两者）。',
    '"已达成"的判定阈值 = 该项预期收益的一半（下限 1%）：例如预期 33%，实测目标指标改善 ≥16.7% 才算达成，避免把噪声当成成果。',
    '针对采集质量而非运行时性能的建议（如"补齐采集"）没有可比指标，会标为"无法判定"，不会算作失败。',
  ],
  phaseAdvice: [
