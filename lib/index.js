...
import { analyzeDataset, compareCaptures } from './analysis/index.js';
...
async function handleApi({ req, res, url, path, config, store, log, assetRoot }) {
  ...
  if (datasetMatch !== null) {
    ...
    if (sub === '/compare' && method === 'GET') {
      const otherId = url.searchParams.get('with');
      if (otherId === null || otherId === '') {
        sendJson(res, 400, { error: '缺少 with 参数：需要指定用于对比的另一份数据集 id' });
        return;
      }
      const other = store.getDataset(otherId);
      if (other === undefined) {
        sendJson(res, 404, { error: '用于对比的数据集不存在或已过期（默认空闲 1 小时后释放）' });
        return;
      }
      const comparison = compareCaptures({
        before: { dataset: record.dataset, analysis: record.analysis, datasetId: datasetMatch[1], label: record.label },
        after: { dataset: other.dataset, analysis: other.analysis, datasetId: otherId, label: other.label },
      });
