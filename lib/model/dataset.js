...
/** CSV table kinds that describe the *same* device work, most authoritative first. */
const CSV_KIND_PRIORITY = Object.freeze([
  'op_statistic',
  'op_summary',
  'kernel_details',
  'operator_details',
 ...
 *
 * A profile normally describes the same work in several tables at once: the CANN
 * per-operator summary (`op_statistic.csv`), per-operator instances
 * (`operator_details.csv`) and per-kernel instances (`kernel_details.csv`). Adding
 * them together counts that work two or three times, which would (a) inflate the
 * totals whenever the CSV is the authoritative source — i.e. exactly when the
 * trace was sampled — and (b) produce a meaningless cross-check delta against the
 * trace. So one kind is chosen per operator as the source of truth (the summary
 * tables first, then the instance tables), the others stay listed as evidence, and
 * a genuine disagreement between kinds is reported instead of being averaged away.
 *
...
function aggregateTables(tables, warnings) {
  ...
  for (const table of tables) {
    ...
    if (!CSV_KIND_PRIORITY.includes(table.kind)) {
      continue;
    }
    if (['op_statistic', 'api_statistic'].includes(table.kind)) statTables.push(table.name);
    else instanceTables.push(table.name);
    const buckets = byKind.get(table.kind) ?? new Map();
    byKind.set(table.kind, buckets);
  ...
  const names = new Set();
  for (const buckets of byKind.values()) for (const name of buckets.keys()) names.add(name);
  for (const name of names) {
    const available = CSV_KIND_PRIORITY.filter((kind) => byKind.get(kind)?.has(name));
    const primaryKind = available[0];
    const primary = byKind.get(primaryKind).get(name);
    // Totals come from one table; the distribution and the pipe-utilization
    // columns may come from another as long as it is a single table as well.
    const distributionKind = available.find((kind) => byKind.get(kind).get(name).durations.length > 0) ?? primaryKind;
    const utilizationKind = available.find((kind) => byKind.get(kind).get(name).utilizationSamples > 0) ?? primaryKind;
    const distribution = byKind.get(distributionKind).get(name);
    const utilization = byKind.get(utilizationKind).get(name);
    const deviceIds = new Set();
    for (const kind of available) for (const id of byKind.get(kind).get(name).deviceIds) deviceIds.add(id);
    byName.set(name, {
      name,
      ...
      role: primary.role,
      count: primary.count,
      totalUs: primary.totalUs,
      waitUs: primary.waitUs,
      durations: distribution.durations,
      sources: new Set(available),
      totalByKind: new Map(available.map((kind) => [kind, byKind.get(kind).get(name).totalUs])),
      deviceIds,
      utilization: utilization.utilization,
      utilizationSamples: utilization.utilizationSamples,
    });
  }

  // Independent tables describing the same device time should broadly agree. A
  // small spread is normal (an operator instance covers its kernels plus their
  // gaps), so only a material disagreement — a partial export, duplicated rows,
  // mixed units — is surfaced, and the chosen table is named.
  for (const bucket of byName.values()) {
    const totals = [...bucket.totalByKind]
      .filter(([kind, total]) => kind !== 'api_statistic' && total > 0);
    if (totals.length < 2) continue;
    const high = Math.max(...totals.map(([, total]) => total));
    const low = Math.min(...totals.map(([, total]) => total));
    if ((high - low) / high <= 0.2) continue;
    warnings.push(`${bucket.name} 的多张 CSV 表累计耗时相差较大（${totals.map(([kind, total]) => `${kind} ${total.toFixed(1)}µs`).join('、')}），`
      + `已按 ${String([...bucket.totalByKind.keys()][0])} 取值；请确认是否有重复导出或产物不完整。`);
  }
