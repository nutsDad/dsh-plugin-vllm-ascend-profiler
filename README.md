...
页面是一个**五步流程**（导入产物 → 概览与定位 → 时序取证 → 占比归因 → 优化行动），顶部步骤条显示当前进度并可点击跳转。三个可视化模块共享**同一个筛选状态**：在泳道图里点选算子、在饼图里点选大类、在条形图里点选排行，效果会同步到其它视图（对应条目高亮、其余淡出），筛选条件以可关闭的标签显示，`Esc` 一键清除。建议卡片上的「在第 3 步查看…」会把对应类别/算子的筛选直接带到泳道图。
...
* 左：**环形饼图**——按算子大类汇总，口径可切「设备侧 / 全部 / Host 侧」，点击扇区或图例即按该大类聚焦泳道图；
* 右：**TopN 降序条形图**——绝对耗时 + 占算子总耗时百分比，N 可调 10/20/30，点击条形即筛选该算子；
* **维度切换**：`累计总耗时`（找"次数多、总量大"的算子）↔ `单次执行耗时`（找"单次就很慢"的算子）；
* 下方数据表给出调用次数、累计、均值、p95、占比与**总计来源**（trace 聚合 / CANN 统计表）及 cross-check 偏差。

**模块三 · 结构化性能优化建议**

固定推理链路，五步全部展示（可折叠，不省略中间推理）；每一步都标注它**消费什么、产出什么**：
...
动效都用来说清"数据是怎么来的"，而不是装饰：载入时 KPI 数字滚动、进度条按阶段推进、泳道图从左到右扫出算子条、饼图环形逐段绘出、条形图按名次生长、进度条与评分条从 0 增长、建议卡片淡入。

* 顶栏 **「动效」开关**（默认开，选择记在 `localStorage`）可一键关闭全部动画；
* 操作系统级 `prefers-reduced-motion: reduce` 同样会被尊重；
* 所有交互都有非动画的等价反馈（文字状态、`aria-pressed`、筛选标签）；
* 键盘：`Esc` 清除筛选 / 关闭弹窗，建议卡片与小节标题可 `Tab` 聚焦并 `Enter` 折叠。
...
**第 4 步 · 占比归因**：环形饼图（大类占比）+ TopN 降序条形图

![耗时占比](docs/screenshots/04-share.png)

**跨模块联动**：点击第 4 步的 `MatMulV2` 条形，第 3 步立即筛选到该算子并显示可关闭的筛选标签

![联动筛选](docs/screenshots/09-linked-filter.png)

**第 5 步 · 优化行动**：① 四类候选打分（全量 / Prefill / Decode 独立判定，可展开证据）

![瓶颈定位](docs/screenshots/05-locate.png)

④ 优化方案（依据 / 动作 / 验证 / 风险 / 收益推算）与 ⑤ 预期收益汇总

![优化方案](docs/screenshots/06-actions.png)

![预期收益](docs/screenshots/07-benefit.png)

完整页面长图见 [`08-full.png`](docs/screenshots/08-full.png)，深色主题见 [`10-dark-share.png`](docs/screenshots/10-dark-share.png)。重新生成：
...
当前状态：**80 个用例全部通过**（CI 在 Node 22 与 24 上跑同一套，见 [`.github/workflows/test.yml`](.github/workflows/test.yml)）。
