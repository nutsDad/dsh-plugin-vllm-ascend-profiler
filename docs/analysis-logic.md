...
导入优化后的采集结果后，宿主侧用 `lib/analysis/compare.js` 把两次分析对比成一份可判定的结论（接口 `GET /api/datasets/<before>/compare?with=<after>`）。**两次采集各自跑同一套解析、同一套门限与同一套打分**，差异只体现在数值上。

### 7.1 指标方向（谁变好、谁只是变了）

每个指标带明确方向，避免把"下降"一律当成改善：

| 方向 | 指标 |
| --- | --- |
| **越低越好** | 每步墙钟、采集窗口、NPU 空闲占比、Host 独占/步、Host 独占占比、Host 忙占比、Host 调度耗时、派发算子数/步、同步类算子/步、通信未掩盖、通信时长/步、设备拷贝耗时、D2H/H2D 每步、设备计算耗时、平均空闲段长度、空闲段数量 |
| **越高越好** | NPU 忙碌率、通信与计算重叠率 |
| **仅作背景（无好坏判定）** | 计算占墙钟、通信次数/步、事件总数、设备算子条数 |

变化幅度 <1% 记作"未变化"；背景类指标只显示前后数值，不参与"达成"判定——**占比类指标的分母是墙钟，优化缩短墙钟会让占比反向变化**，因此这类反向变化会单独告警。

### 7.2 建议达成校验

第 5 步的每条建议都声明了它要改善的指标（`ADVICE_TARGETS` 映射，含 `host.enable-graph-mode → 派发算子数/步 + Host 独占/步 + 每步墙钟` 等）。判定规则：

```
observedPct  = 目标指标按"改善方向"折算后的相对变化
thresholdPct = max(1%, 预期收益 × 0.5)
verdict      = observed ≥ threshold → 已达成
             | 0 < observed < threshold → 部分达成
             | observed ≤ 0 → 未达成
             | 无对应指标（如"补齐采集"类建议）→ 无法判定
```

阈值同时写进返回载荷，报告与页面都直接引用它，不需要读者猜"多少算达成"。此外会记录该建议在优化后是否**仍然被列出**（仍被列出说明目标指标未过门限）。

### 7.3 可比性检查（防止把两份不同负载当成优化效果）

下列任一情况都会被明确标注，其中前六项会让 `comparable = false`：

| 检查 | 触发条件 | 为什么重要 |
| --- | --- | --- |
| 步数差异 | 相差 >15% | 逐步指标已按步归一化，但总窗口与占比类指标可比性下降 |
| 阶段构成 | Prefill/Decode 墙钟占比相差 >10pt | 混合负载下"提升"可能只是负载变了 |
| 阶段口径 | 两侧 `phaseOverride` 不同 | 两侧根本不是同一口径 |
| 采样 | 任一侧 trace 被采样 | 累计耗时来源与泳道图保真度不同 |
| 产物集合 | 两侧文件清单不同 | 缺失的表会让对应指标不可比 |
| 耗时口径来源 | 两侧 `totalsSource` 不同 | trace 口径 vs CANN 统计表口径 |
| 瓶颈类型变化 | 主导瓶颈类型不同 | 可能是优化推进了瓶颈，也可能是换了负载（提示确认采集条件） |
| 采集窗口变化 | 相差 >20% | 占比类指标分母变化，判断以 µs 绝对值为准 |

### 7.4 输出结构

```
sides        两侧数据集标签 / 事件数 / 窗口 / 步数 / 瓶颈 / 口径来源
headline     结论句 + 每步墙钟、NPU 忙碌率、Host 独占/步、派发算子数/步 的前后值
metrics      23 项指标的前后值、绝对/相对变化、改善幅度与好坏判定
categories   大类耗时前后对比 + 占比前后 + 绝对变化（按变化量排序）
operators    节省最多的 8 个算子 + 增加最多的 8 个算子（含前后均值）
bottleneck   瓶颈类型是否改变、得分升降
recommendations  每条建议的 verdict / 目标指标 / 实测改善 / 判定阈值 / 是否仍被列出
comparability + warnings  可比性等级、逐条说明与结论边界
```
