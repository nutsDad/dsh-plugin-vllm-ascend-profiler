...
| 总计来源 | 该行累计耗时的来源 | trace 采样时优先取 CSV，否则取 trace | 两者都保留（`traceTotalUs` / `csvTotalUs`），并给出 cross-check 偏差；偏差大意味着 trace 过滤或采样丢算子 |
...
| 大类占比（饼图） | 按计算/通信/拷贝/调度汇总 | 口径可切「设备侧 / 全部 / Host 侧」 | 分母是所选口径下的算子总耗时，页面会显示该数值 |
